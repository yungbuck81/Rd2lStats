...
class Rd2lStats:
    ...
    def get_fantasy_score(self, player):
        ...
        roshans = (player["roshan_kills"] if "roshan_kills" in player else player["roshans_killed"])
        ...
        obs_placed = player["obs_placed"] if player["obs_placed"] is not None else 0
        sentry_kills = player["sentry_kills"] if (
                "sentry_kills" in player and player["sentry_kills"] is not None) else 0
        obs_kills = player["observer_kills"] if (
                "observer_kills" in player and player["observer_kills"] is not None) else 0
        camps_stacked = player["camps_stacked"] if player["camps_stacked"] is not None else 0
        stuns = player["stuns"] if player["stuns"] is not None else 0
        try:
            fantasy_score = round(
                (player["kills"] * fantasy_kill_multiplier) + (player["deaths"] * fantasy_death_multiplier) + (
                        player["assists"] + fantasy_assist_multiplier) + (
                        player["last_hits"] * fantasy_lasthit_multiplier) + (
                        player["gold_per_min"] * fantasy_gpm_multiplier) + (
            ...
            for player, gpm in copy.deepcopy(gpmdict).items():
                if player in gpm_player_set:
                    ...
                    if uinput == 1:
                        ...
                        evalList = [(cumulative + (gpm[0] * gpm[1])) / 2, matches]
        ...
        for index, kdadict in enumerate(
            ...
            for player, kda in copy.deepcopy(kdadict).items():
                if player in kda_player_set:
                    print("-- Player KDA: {} found in {} and {} -- ".format(player, kda_player_set[player], index + 1))
                    uinput = int(input("Which position should be selected? 1,2,3,4,5 for roles or 0 to keep separate"))
                    if uinput == 1:
                        cumulative = kda_value_set[player][0] * kda_value_set[player][1]
                        matches = kda_value_set[player][1] + kda[1]
                        evalList = [(cumulative + (kda[0] * kda[1])) / 2, matches]
        ...
        for index, fantasydict in enumerate(
            ...
            for player, fantasy in copy.deepcopy(fantasydict).items():
                if player in fantasy_player_set:
                    print("-- Player Fantasy: {} found in {} and {} -- ".format(player, fantasy_player_set[player],
                                                                                index + 1))
                    uinput = int(input("Which position should be selected? 1,2,3,4,5 for roles or 0 to keep separate"))
                    if uinput == 1:
                        cumulative = fantasy_value_set[player][0] * fantasy_value_set[player][1]
                        matches = fantasy_value_set[player][1] + fantasy[1]
                        evalList = [(cumulative + (fantasy[0] * fantasy[1])) / 2, matches]
                        fantasy1[player] = evalList
                        fantasydict.pop(player)
                    elif uinput == 2:
                        cumulative = fantasy_value_set[player][0] * fantasy_value_set[player][1]
                        matches = fantasy_value_set[player][1] + fantasy[1]
                        evalList = [(cumulative + (fantasy[0] * fantasy[1])) / 2, matches]
    ...
    def get_stats(self):
        ...
        time.sleep(60)
        response = requests.get(opendota_api_players_url + self.highest_gpm_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_xpm_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_kda_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_camps_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_herodamage_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_stuns_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_towerdamage_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_lane_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_deward_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_apm_player)
        json_data = json.loads(response.text)
        ...
        response = requests.get(opendota_api_players_url + self.highest_courier_player)
        json_data = json.loads(response.text)
        ...
        stats_leaders_file = open('../stats_leaders.csv', 'w', encoding="utf-8", newline='')
